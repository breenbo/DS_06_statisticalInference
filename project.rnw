...
<<plots, echo=TRUE, size="small", fig.height=4, cache=TRUE>>=
library(ggplot2)
library(cowplot)
...
<<echo=3:8, size="small", cache=TRUE>>=
library(datasets)
library(dplyr)
...
<<echo=TRUE, size="small", cache=TRUE>>=
oj05 <- ToothGrowth %>% filter(supp=="OJ") %>% filter(dose==0.5)
vc05 <- ToothGrowth %>% filter(supp=="VC") %>% filter(dose==0.5)
t.test(oj05$len-vc05$len, alternative="greater")
oj1 <- ToothGrowth %>% filter(supp=="OJ") %>% filter(dose==1)
vc1 <- ToothGrowth %>% filter(supp=="VC") %>% filter(dose==1)
t.test(oj1$len-vc1$len, alternative="greater")
oj2 <- ToothGrowth %>% filter(supp=="OJ") %>% filter(dose==2)
vc2 <- ToothGrowth %>% filter(supp=="VC") %>% filter(dose==2)
