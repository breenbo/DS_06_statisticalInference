...
\hlstd{ToothGrowth} \hlopt{%>%} \hlkwd{group_by}\hlstd{(supp,dose)} \hlopt{%>%} \hlkwd{summarise}\hlstd{(}\hlkwd{mean}\hlstd{(len))}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{count}\hlstd{(ToothGrowth, supp, dose)}
\end{alltt}
\begin{verbatim}
## Source: local data frame [6 x 3]
## Groups: supp [?]
...
\end{verbatim}
\begin{alltt}
\hlkwd{data.frame}\hlstd{(}\hlkwc{mean}\hlstd{=}\hlkwd{mean}\hlstd{(ToothGrowth}\hlopt{$}\hlstd{len),} \hlkwc{sd} \hlstd{=} \hlkwd{sd}\hlstd{(ToothGrowth}\hlopt{$}\hlstd{len))}
\end{alltt}
\begin{verbatim}
##       mean       sd
## 1 18.81333 7.649315
